...
@fixture
def combiner_tech_config_electricity_3_in():
    elec_combiner_dict = {
        "model_inputs": {
...
def test_generic_combiner_performance_power_3_in(
    plant_config, combiner_tech_config_electricity_3_in
):
    prob = om.Problem()
    comp = GenericCombinerPerformanceModel(
        plant_config=plant_config,
        tech_config=combiner_tech_config_electricity_3_in,
        driver_config={},
    )
    prob.model.add_subsystem("comp", comp, promotes=["*"])
    ivc = om.IndepVarComp()
    ...
    electricity_output = electricity_input1 + electricity_input2 + electricity_input3
