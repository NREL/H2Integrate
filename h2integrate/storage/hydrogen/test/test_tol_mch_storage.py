...
def test_mch_ex1(subtests):
    in_dict = {
        "max_H2_production_kg_pr_hr": 14118.146788766157,
        "hydrogen_storage_capacity_kg": 2081385.9326778147,
        "hydrogen_demand_kg_pr_hr": 10775.824040553021,
        "annual_hydrogen_stored_kg_pr_yr": 17878378.49459929,
    }
    mch_storage = MCHStorage(**in_dict)
    cost_res = mch_storage.run_costs()

    with subtests.test("CapEx"):
        assert pytest.approx(cost_res["mch_capex"], rel=1e-6) == 2.62304217 * 1e8
    with subtests.test("Fixed O&M"):
        assert pytest.approx(cost_res["mch_opex"], rel=1e-6) == 7406935.548022923
    with subtests.test("Variable O&M"):
        assert pytest.approx(cost_res["mch_variable_om"], rel=1e-6) == 9420636.00753175
