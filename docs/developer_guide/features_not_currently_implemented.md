...
- specifying resource-specific locations when running a design sweep that includes latitude and longitude as design variables. Currently, sweeping locations can only be done if the location is the same for each resource model.
