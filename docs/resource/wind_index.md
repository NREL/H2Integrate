...
```{note}
Not all wind resource models will output all the data keys listed below. Please check the documentation on each wind resource model and wind performance model to ensure compatibility.
```

### Primary Data: Wind Resource Timeseries
The below variables are output as arrays, with a length equal to the simulation duration. In the variabled listed below, `<height>` is used to indicate the height (in meters) above the ground for the resource parameter. `<height>` should be 0 for surface level wind resource data. The naming convention and standardized units of wind resource variables are listed below:
...
Time data may be output as arrays to represent the time profile of the resource data. These times should be represented in the timezone of `data_tz` (if output).
