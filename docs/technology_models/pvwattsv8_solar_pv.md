...
$$
\text{PV Capacity (kW-AC)} = \frac{\text{PV Capacity (kW-DC)}}{dc\text{\underline{ }}ac\text{\underline{ }}ratio}
$$
...
- `tilt_angle_func` (optional): options are `"none"`, `"lat-func"` or `"lat"` and defaults to `"none"`.
    ...
    \theta =
    \begin{cases}
    \text{lat}\cdot0.87 \quad \text{ if } \text{lat} \leq 25 \\
    3.1 + \text{lat}\cdot0.76 \quad \text{ if } 25<\text{lat} \leq 50 \\
    \text{lat} \quad \text{ if } 50 < \text{lat} \\
    \end{cases}
    $$
