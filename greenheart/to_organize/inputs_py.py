...
def establish_save_output_dict():
    ...
    save_outputs_dict[
        "H_LCOE HVDC scenario no operating costs (uses LCOE calculator) ($/kg)"
    ] = list()
    save_outputs_dict[
        "H_LCOE HVDC scenario w/ operating costs (uses LCOE calculator) ($/kg)"
    ] = list()
    save_outputs_dict[
    ...
    save_outputs_dict[
        "Levelized Cost/kg H2 HVDC scenario (CF Method - using annual cashflows per technology) ($/kg)"
    ] = list()
    save_outputs_dict[
        "Levelized Cost/kg H2 Pipeline scenario (CF Method - using annual cashflows per technology) ($/kg)"
    ] = list()
    save_outputs_dict[
    ...
    save_outputs_dict[
        "Total Yearly Electrical Generation used by Electrolyzer (kWh)"
    ] = list()
...
def save_the_things():
    ...
    save_outputs_dict["Total Annual H2 production (kg)"] = H2_Results[
    ...
    save_outputs_dict["Electrolyzer Capacity Factor (%)"] = (
        H2_Results["cap_factor"] * 100
    )
    save_outputs_dict["HOPP Annual Energy Shortfall (kWh)"] = np.sum(
