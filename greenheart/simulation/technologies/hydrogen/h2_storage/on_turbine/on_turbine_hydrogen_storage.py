...
class PressurizedTower:
    def __init__(self, year: int, turbine: dict):
        # key inputs
        self.year = year
        ...
        self.density_steel = 7817.0  # kg/m^3
        self.gasconstant_H2 = 4126.0  # J/(kg K)
        self.operating_temp = 25.0  # degC
        ...
        self.operating_pressure = PressurizedTower.get_crossover_pressure(
            self.welded_joint_efficiency, self.ultimate_tensile_strength, self.d_t_ratio
        )
    ...
    def run(self):
        ...
        self.wall_material_mass = self.wall_material_volume * self.density_steel
        self.cap_material_mass = (
            self.cap_bot_material_volume + self.cap_top_material_volume
        ) * self.density_steel
        ...
        if False:
            ...
            print()
            print(
                "tower wall material volume (non-pressurized):",
                self.wall_material_volume_trad,
            )
            print(
                "tower wall material mass (non-pressurized):",
                self.wall_material_mass_trad,
            )
            print(
                "tower wall material cost (non-pressurized):",
                self.wall_material_cost_trad,
            )
            print(
                "tower cap material volume (non-pressurized):",
                self.cap_top_material_volume_trad + self.cap_bot_material_volume_trad,
            )
            print(
                "tower cap material mass (non-pressurized):",
                self.cap_material_mass_trad,
            )
            print(
                "tower cap material cost (non-pressurized):",
                self.cap_material_cost_trad,
            )
            print(
                "tower total material cost (non-pressurized):",
            ...
            print()
            print(
                "tower cap material volume (pressurized):",
                self.cap_bot_material_volume + self.cap_top_material_volume,
            )
            ...
            print(
                "tower total material cost (pressurized):",
                self.wall_material_cost + self.cap_material_cost,
            )

            print()
            print(
                "delta tower wall material cost:",
    ...
    def get_volume_tower_inner(self):
        ...
        for i_section in range(Nsection):
            ...
            vol_section[i_section] = PressurizedTower.compute_frustum_volume(
                dh, diameter_bot, diameter_top
            )

        # total volume: sum of sections
        return np.sum(vol_section)  # m^3

    def get_volume_tower_material(self, pressure: float | None = None):
        """
        ...
        alpha_dtp = PressurizedTower.get_thickness_increment_const(
        ...
        for i_section in range(Nsection):
            ...
            if self.setting_volume_thickness_calc == "centered":
                Vouter = PressurizedTower.compute_frustum_volume(
                    h2 - h1,
                    d1 * (1 + (1 / self.d_t_ratio + alpha_dtp)),
                    d2 * (1 + (1 / self.d_t_ratio + alpha_dtp)),
                )
                Vinner = PressurizedTower.compute_frustum_volume(
                    h2 - h1,
                    d1 * (1 - (1 / self.d_t_ratio + alpha_dtp)),
                    d2 * (1 - (1 / self.d_t_ratio + alpha_dtp)),
                )
        ...
        if pressure == 0:
            ...
            Vmat_bot = (np.pi / 4 * self.section_diameters[0] ** 2) * (
                PressurizedTower.compute_cap_thickness(
                    pressure,
    ...
    def get_mass_tower_material(self, pressure: float | None = None):
        """
    ...
    def get_cost_tower_material(self, pressure: float | None = None):
        ...
        if pressure == 0:
            ...
            return [
                ...
                self.costrate_endcap * Mmat_bot,
                self.costrate_endcap * Mmat_top,
            ]  # 2003 dollars
    ...
    def get_cost_nontower(self, traditional: bool = False, naive: bool = True):
        nonwall_cost = 0
        if traditional:
            ...
            if naive:
                ...
                adj_length = np.sqrt(
                    (self.section_diameters[0] - self.section_diameters[-1]) ** 2
                    + self.tower_length**2
                )
                nonwall_cost += self.cost_mainframe_extension
                nonwall_cost += 2 * self.cost_door
    ...
    def get_capex(self):
        ...
        capex_without = self.get_cost_nontower(traditional=True) + np.sum(
            self.get_cost_tower_material(pressure=0)
        )
    ...
    def get_opex(self):
        ...
        return (
            self.get_capex() * self.maintenance_rate + self.wage * self.staff_hours
        )  # 2003 dollars
    ...
    def get_pressure_H2(self):
        return self.operating_pressure  # Pa, trivial, but for delivery

    ### STATIC FUNCTIONS

    @staticmethod
    def compute_cap_thickness(
        pressure,
        diameter,
        strength_yield,
        safetyfactor_Sy=1.5,
        efficiency_weld=0.80,
        constant=0.10,
    ):
        """
        ...
        return diameter * np.sqrt(
            constant * pressure / (efficiency_weld * strength_yield / safetyfactor_Sy)
        )
    ...
    def compute_frustum_volume(height, base_diameter, top_diameter):
        ...
        return (
            ...
            / 12.0
            * height
            * (base_diameter**2 + base_diameter * top_diameter + top_diameter**2)
        )  # volume units

    @staticmethod
    def get_crossover_pressure(
        ...
        p_crossover = 4 * E * Sut / (7 * d_over_t * (1 - E / 7.0))  # pressure units
